#!/usr/bin/env python3
"""Visualize before/after smoothing hand 10D state/action curves."""
...
from visualize_smooth_eef9d_comparison import (
    DEFAULT_QUALITY_SUMMARY,
    DEFAULT_SMOOTH_DATASET,
    DEFAULT_SMOOTH_SUMMARY,
    DEFAULT_BEFORE_DATASET,
    MISSION_DIR,
    compact_list,
    compact_matrix,
    data_path_for_episode,
    feature_names,
    finite_mean,
    read_json,
    read_jsonl,
...
DEFAULT_OUTPUT = MISSION_DIR / "batch_quality" / "mission2_smooth_all" / "hand10_quality_report.html"
...
<head>
  ...
  <title>Mission2 Smooth Hand 10D Quality</title>
  <style>
    ...
    .grid { display: grid; grid-template-columns: repeat(2, minmax(340px, 1fr)); gap: 12px; align-items: start; }
    ...
    @media (max-width: 760px) { body { margin: 14px; } select { min-width: 0; width: 100%; } }
  </style>
</head>
<body>
  <h1>Mission2 Smooth Hand 10D Quality</h1>
  <p class="note">
    Before dataset: <code id="before-path"></code>.
    Smooth dataset: <code id="smooth-path"></code>.
    Hand state/action are read from LeRobot parquet hand joint slices in canonical L10 order.
    State curves use the left y-axis, action curves use the right y-axis.
  </p>
  ...
  <script>
    ...
    const dimCount = Number(payload.dim_count || labels.length || 10);
    ...
    addSummary('Action mean change', fmt(payload.smooth_summary?.action_mean_abs_change_hand));
    addSummary('State mean change', fmt(payload.smooth_summary?.state_mean_abs_change_hand));
...
def hand_indices(info: dict[str, Any], modality: dict[str, Any]) -> tuple[list[int], list[int]]:
    state_slice = slice_from_modality(modality, "state", "hand_joint_pos")
    action_slice = slice_from_modality(modality, "action", "hand_joint_target")
    if state_slice is not None and action_slice is not None:
        ...
        if state_end - state_start == 10 and action_end - action_start == 10:
            return list(range(state_start, state_end)), list(range(action_start, action_end))

    state_names = tuple(f"hand_joint_pos.{name}" for name in HAND10_LABELS)
    action_names = tuple(f"hand_joint_target.{name}" for name in HAND10_LABELS)
    state_lookup = {name: index for index, name in enumerate(feature_names(info, "observation.state"))}
    action_lookup = {name: index for index, name in enumerate(feature_names(info, "action"))}
    if not all(name in state_lookup for name in state_names) or not all(name in action_lookup for name in action_names):
        raise RuntimeError("Could not resolve hand 10D indices from modality.json or feature names.")
    return [state_lookup[name] for name in state_names], [action_lookup[name] for name in action_names]
...
def load_hand10(
    ...
    if state.shape[1] != 10 or action.shape[1] != 10:
        raise RuntimeError(f"{data_path} did not resolve to 10D hand state/action arrays.")
...
def episode_payload(
    ...
    before_path, before_df, before_state, before_action = load_hand10(
        before_dataset,
        before_info,
        episode,
        before_indices[0],
    ...
    smooth_path, smooth_df, smooth_state, smooth_action = load_hand10(
        smooth_dataset,
        smooth_info,
        episode,
        smooth_indices[0],
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    before_indices = hand_indices(before_info, before_modality)
    smooth_indices = hand_indices(smooth_info, smooth_modality)
    payload_episodes = [
        episode_payload(
            before_dataset,
            smooth_dataset,
    ...
    return {
        ...
        "dim_labels": list(HAND10_LABELS),
        "dim_count": len(HAND10_LABELS),
