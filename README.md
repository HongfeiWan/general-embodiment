...
- smooth 全量日期分布质量分析：`tools/data_chain/analyze_smooth_quality_all_dates.py`
...
示例：对 canonical smooth 数据集做全量质量检查，并用来源日期配色比较不同采集日的分布差异：
...
主报告输出到 `missions/nero/mission2/smooth_quality/all_dates/smooth_quality_all_dates_report.html`。
